...
@Injectable()
export class TrackGrpc implements OnModuleInit {
  private readonly logger = new Logger(TrackGrpc.name);
  private rawService: TrackServiceClient;
  
  public client: GrpcToPromise<TrackServiceClient>; 

  constructor(
    @Inject('TRACK_INTERNAL_PROXY') private readonly grpcClient: ClientGrpc,
  ) {}

  onModuleInit() {
    // Original gRPC service from Nest
    this.rawService = this.grpcClient.getService<any>('TrackService');
